...
const Profile = props =>{
  ...
  let {property,picts,loading,enableToast,toastMessage} = props;
  let refSlug = property.slug;
  console.log(toastMessage);
  return(
    <Formik initialValues={property}  onSubmit={(values, {setSubmitting})=>{
          dispatch({type:'add_loader',payload:'load'});
          values.picture=JSON.stringify(picts);
          dispatch(productUpdator(values,refSlug));
         }}
       ...
       return(
       <form onSubmit={handleSubmit} loadnext={property?property.title:''}>
        ...
        <div class="col-md-12 form-group">
            <input type="text" className="form-control form-control-user" id="slug" name="slug" placeholder="Slug" value={values.slug} onChange={handleChange} onBlur={handleBlur} disabled={true}/>
